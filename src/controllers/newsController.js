...
const newsController = {
  ...
  userGetNews: async (req, res) => {
    ...
    try {
      ...
      const newsData = await News.find({ author: userId })
        .sort({ _id: -1 })
        .limit(limit);
      res.json(newsData);
    } catch (error) {
  ...
  userUpdateNews: async (req, res) => {
    try {
      ...
      const existingNews = await News.findById(id).where({ author: userId });
      if (!existingNews) {
        return res.status(404).json({ message: 'Không tìm thấy tin tức' });
      }
  ...
  userDeleteNews: async (req, res) => {
    try {
      ...
      const { id } = req.params;
      const news = await News.findById(id).where({ author: userId });
      if (!news) {
        return res.status(404).json({ message: 'Không tìm thấy tin tức' });
      }
